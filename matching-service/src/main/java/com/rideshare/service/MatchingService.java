...
import com.rideshare.client.LocationServiceClient;
import com.rideshare.dto.NearByDriverResponse;
import com.rideshare.event.RideMatchedEvent;
...
public class MatchingService {
    ...
    public void matchDriverForRide(RideRequestedEvent event) {
        ...
        if (nearByDrivers.isEmpty()) {
            log.warn("No drivers found near ridee");
            return;
        }
        ...
        if (bestDriver.isEmpty()) {
            log.warn("Could not find suitable driver for the ride");
            return;
        }
        ...
        kafkaTemplate.send(RIDE_MATCHED_TOPIC, rideMatchedEvent);
        log.info("RideMatchedEvent published to topic");
    }
